...
client.on("guildMemberAdd", member => {
  member.roles.add('775850761556983808'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "775851274394402886");
    const register = "<@&775850756535615538>"
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = '∾'
  const sunucu = '754446402217836696'
  const kanal = '775850889793634374'
  const rol = '775850756955832331'
...
client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get('754446402217836696'); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var tag = "∾"; // Buraya Ekip Tag
  var tagrol = "775850756955832331"; // Buraya Ekip Rolünün ID
  var kanal = "775850889793634374"; // Loglanacağı Kanalın ID
